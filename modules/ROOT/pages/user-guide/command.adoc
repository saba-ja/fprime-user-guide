...
6. `priority`: For async commands, you may specify a priority. The priority specification is not allowed for other kinds of ports. The priorities regulate the order in which elements are dispatched from the message queue.

7. Queue Full Behavior: For async commands, you may specify a what should the system do when the message queue is full. There are three possible behaviors:
